...
public class Task03 extends JFrame implements ActionListener {
	...
	public Task03() {
		...
		inputfld.setText("텍스트필드에 임시값 표시~");
	...
	public void actionPerformed(ActionEvent e) {
		//1. String s에 액션 결과 연결 = 값을 콘솔에 출력 가능 
		s = s + e.getActionCommand(); 
		
		System.out.println(s);
		
		//2. Back-한 칸 삭제(이벤트.이벤트결과메소드.이글자를뽑아봐 && 텍스트길이가 0보단 커야함) 
		if(e.getActionCommand().equals("Backspace") && s.length()>0) {
			s = inputfld.getText(); 
			if(s.length() > 0) {
	        ...
	        s = ""; // 입력 초기화
	        input_Act = e.getActionCommand();  // 클릭된 연산자 저장
			...
			inputfld.setText(String.valueOf(result)); 
			s = String.valueOf(result);
		} else if(e.getActionCommand().equals("C") && s.length()>0) {
